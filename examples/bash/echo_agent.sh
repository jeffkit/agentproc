#!/usr/bin/env bash
# Minimal AgentProc-compliant agent (Bash, wire 0.4).
...
#   timeout_secs: 10
#
# Wire 0.4: the turn object arrives on stdin as one NDJSON line; the agent
# replies with a single {"type":"result"} event on stdout.
read -r turn
