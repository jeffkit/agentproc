...
- **Placeholder substitution**: `{{SESSION_ID}}`, `{{SESSION_NAME}}`, `{{PROFILE_DIR}}` in `command`, `args`, `cwd`, and `env` values — no shell involved. `{{MESSAGE}}` is not a placeholder; the message travels via stdin.
