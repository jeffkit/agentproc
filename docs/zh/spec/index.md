...
send_error_reply: true        # agent 出错时通知用户
...
最终 argv（`[command, *args]`）直接传给 `execve`，避免 `{{MESSAGE}}` 引起的 shell 注入，也允许 `command` 携带含空格的路径：
...
当 `send_error_reply: true` 且进程非零退出又未发出 `error` 事件时，bridge 向用户发送通用错误消息。
