...
│   → 看：spawn ENOENT
│
├─ stderr 上出现 "AGENT_ERROR:..."
│   → 是被包装的 agent 自己报错。看：被包装 CLI 的 AGENT_ERROR
│
├─ 卡住 / 无输出
│   → 看：agent 跑了但没返回内容
│
...
- 改用 Node bridge 脚本（`command: node {{PROFILE_DIR}}/bridge.js`）——Node 一定能找到（CLI 本身就跑在 Node 上）。
...
你在跑一个旧版 hub profile（或手改过的）——`command: python3 ./bridge.py` 用了相对路径，但 agent 的 `cwd` 里没这个文件。

**修法：** 重新安装或刷新 profile（新版 profile 用 `{{PROFILE_DIR}}/bridge.py`，永远能正确解析）：
```bash
...
被包装的 CLI 跑起来了，但自己报错。bridge 通过 `AGENT_ERROR:` 把错误转发给你（协议规定）。CLI 同时在 stderr 上以 `agentproc:error:<message>` 形式展示。
...
1. **流式模式下，agent 的回复全部通过 `AGENT_PARTIAL:` 行输出。** 用了 `--quiet` 时 partial 被压制，你什么都看不到。重新跑时去掉 `--quiet`，或从 stderr 的 `agentproc:session:` 行确认 agent 确实回复了。
...
3. **agent 退出码 0 但什么都没写。** 这是 agent 脚本本身的 bug，不是 AgentProc 的问题。直接用环境变量驱动 agent 看它实际行为：
   ```bash
