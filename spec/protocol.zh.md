...
**线协议（Wire protocol）：** `0.3`（由 turn 对象的 `protocol_version` 字段携带的字符串）
**文档修订：** `1.0`
**状态：** 草案
...
理由：任何可比较的版本号都会诱导实现者用 `>= 0.3` 来 gate 行为，而一旦某个 bridge 没有同步 bump 数字，这种判断就会失效。把字符串视为不透明，能让契约保持诚实：某项能力的存在由承载它的字段来表示。
...
- **输入 — 环境变量：** 密钥与配置（profile 的 `env` 块，外加一组最小 infra 集合）。0.3 中**每轮请求不再走 env 变量**——它走 stdin 的 turn 对象。
...
| `protocol_version` | string | 协议版本字符串，例如 `"0.3"`。**不透明且不可比较**——见[版本治理](#版本治理)。 |
...
| `text` | agent → bridge | 最终回复正文。多个 `text` 事件按序拼接。 |
| `session` | agent → bridge | 声明或更新会话 ID。最后一行生效。 |
| `error` | agent → bridge | 终止性错误消息，转发给用户。 |
| `permission_request` | agent → bridge | 可选的工具授权请求（仅当 `permission: true`）。 |

### 封闭词汇表

上述事件集合是**封闭**的。七个 `type` 值——`turn`（stdin）、`partial`、`text`、`session`、`error`、`permission_request` 与 `permission_response`（stdin）——是整个协议的全部词汇。AgentProc 故意**不**为工具调用、文件 diff、计划更新、推理块等更丰富语义增长类型化事件。需要这些的 agent 应在内部封装一个面向 IDE 的协议（例如 ACP）；AgentProc bridge 不渲染 diff、不持有用户文件，因此与这类事件无关。bridge **MUST NOT** 期待额外事件类型，agent **MUST NOT** 发明新类型来向本协议走私更丰富的语义——那条路只会把 ACP 实现得很糟。未知 `type` 按[格式错误的行](#格式错误的行)处理。
...
当 profile 设了 `streaming: false` 时，bridge 忽略所有 `partial` 事件，仅从 `text` 事件拼装回复。
...
| 字段 | 类型 | 描述 |
|-------|------|-------------|
| `type` | string | 字面量 `"text"`。 |
| `text` | string | 最终回复正文的一段。可含换行。 |

一个 turn 中多个 `text` 事件按序拼接成最终回复正文，在进程退出后（若非空）发送给用户。若所有内容已通过 `partial` 事件投递且未发 `text` 事件，则最终回复为空，bridge 跳过最终发送。

一个未产生 `text`（且未产生 `partial`）但退出 `0` 的 turn 是**成功**的 turn（例如把一切通过 `partial` 转发后即退出的 agent）。空输出只有在搭配非零退出码或 `error` 事件时才是失败。
...
| 字段 | 类型 | 描述 |
|-------|------|-------------|
| `type` | string | 字面量 `"session"`。 |
| `id` | string | 会话 ID，作为 JSON 字符串携带。 |

**会话事件规则（消解顺序歧义）：**

- 会话事件 **MAY** 出现在 stdout 的**任意位置**——首行、与 partial 交错、或末行。
- 若发出多个 `session` 事件，**最后一行生效**。bridge 存下最终值并在下一轮作为 `session_id` 传回。
- 此规则照顾了会话 ID 仅在底层 CLI 退出后才已知的常见情形（例如 `claude --output-format stream-json` 在终止 `result` 事件中发出会话 ID）。
- **与 `error` 的交互** —— CLI 的终止事件常同时携带会话 ID 和错误指示（例如 `result{session_id, is_error: true}`）。当同一 turn 中同时出现 `session` 事件和 `error` 事件时，bridge **MUST** 仍为下一轮保留会话 ID，即便当前 turn 对用户上报为失败。error 终止 turn；不使会话失效。已发出 `error` 且已学到会话 ID 的 agent **SHOULD** 先发 `session` 事件（或任意位置——bridge 按 last-wins 处理）。

**线上级与存储级 ID 约束。** 线上 `id` 是任意 JSON 字符串——**MAY** 含冒号、斜杠、空格或任何可 JSON 转义的字符。（0.3 取消 0.2 的字符集限制，因为不再有冒号分隔的行前缀会与之冲突。）但把每个会话存为 `<id>.jsonl` 的 SDK 历史助手施加了**存储级**约束：作为文件名组件不安全的 ID（例如含 `/`、`..` 或 NUL）无法在文件存储中往返。持久化 ID 的 bridge/SDK **SHOULD** 清洗或拒绝此类 ID 并记 stderr 警告，保留先前捕获的 ID；线上本身不加限制。

此事件由 bridge 消费，**不**出现在发给用户的回复中。
...
与 `error` 事件一同产生的任何 `text` 事件被丢弃。

**SDK 约定。** 封装本协议的 SDK（例如官方 Python 与 Node 的 `create_profile`/`createProfile` 助手）把 `send_error()` 视为**终止性**：agent **SHOULD NOT** 在调用它之后再发 `partial` 或 `text`。SDK **MAY** 通过在 `send_error()` 后立即退出来强制。这比原始协议更严格（协议允许 agent 发 `error` 后继续写——bridge 只是丢弃余下内容），但这是推荐的 SDK 习惯，因为把错误与后续内容混在一起会令用户困惑。
...
不是合法 JSON、是合法 JSON 但非对象、或缺少受认可 `type` 的 stdout 行属于协议违规。bridge **SHOULD** 记 stderr 警告并**忽略该行**。它**不**作为回复正文转发给用户——0.3 中回复正文仅由 `text` 事件携带。（这比 0.2 更严格，0.2 把任何无前缀的行视为正文。代价是手写的 `echo "hello"` shell agent 不再是合法 agent；见[设计理由](#设计理由)。）
...
- **不是：** 通用的 human-in-the-loop 问答协议。澄清问题应放在正常的 `text` / `partial` 内容中；用户在**下一** IM turn 回答。因此在无头 IM bridge 中禁用交互式问卷工具（例如 Claude Code 的 `AskUserQuestion`）是合理且推荐的。
...
- `partial` / `text` **MAY** 在同一 turn 的 permission 流量前后出现。
- `error` 仍使 turn 失败。挂起的 permission 请求作废；bridge **SHOULD** 停止等待用户批准。
- `session` last-wins 不变；会话事件 **MAY** 在 permission 流量前后出现。
...
| 码 | 含义 |
|------|---------|
| `0` | 成功——stdout 内容（由 `text` 事件和已转发 `partial` 事件拼装）作为回复发送 |
| `1` | 通用 agent 错误 |
...
0.3 让每个 stdout 行成为带 `type` 字段的 JSON 对象。这移除了冲突规则（正文现在是 `{"type":"text"}`），统一了编码（`partial` 和 `text` 都携带 `text` 字符串字段），并让 `partial` 能长出 `role` 等字段。代价是裸 `echo "hello"` 不再是合法 agent——它必须发 `{"type":"text","text":"hello"}`。真实 agent 都是封装脚本（每个 hub profile 都是围绕底层 CLI 的 Python 或 Node 封装，而底层 CLI 内部已经在发 NDJSON），所以它们本来就在做 JSON；唯一失去的是 5 分钟 bash 冒烟测试，而一段 3 行的 Python/Node 脚本即可替代。
...
可调试性几乎不变：`AGENT_MESSAGE="hello" ./agent.sh` 变成 `echo '{"type":"turn","message":"hello","session_id":"","from_user":"u1","protocol_version":"0.3"}' | ./agent`——仍是一行，只是不再是 env 赋值。
...
因为底层 CLI 常常直到退出才知道自己的会话 ID。`claude --output-format stream-json` 在终止 `result` 事件中发出会话 ID，那是运行的最后一个事件。「必须首事件」规则会迫使 bridge 作者陷入尴尬的缓冲。「last wins」让 agent 在得知会话 ID 时随时发出。
...
**与 AgentProc 的关系：** **0.3 起同为线上格式。** AgentProc 0.3 双向都是 NDJSON：stdin 上一行一个 JSON 对象（turn，其后可选 permission 响应），stdout 上也是（事件）。与裸 NDJSON 的区别在于固定、小的事件词汇表（`turn` / `partial` / `text` / `session` / `error` / `permission_request` / `permission_response`）以及每 turn 一进程的生命周期，而非长生命周期双向 RPC 流。
...
**与 AgentProc 的关系：** **哲学基础。** AgentProc 用 filter 没有的两样东西扩展了 Unix filter 约定：会话连续性交接（`{"type":"session"}`）和流式事件（`{"type":"partial"}`）。其余都是普通 Unix。0.3 让 filter 变成「JSON 行进、JSON 行出」而非裸文本，但形状仍是 filter。
