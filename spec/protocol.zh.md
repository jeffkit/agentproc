...
- **输入 — argv 占位符：** `{{SESSION_ID}}`、`{{SESSION_NAME}}`、`{{PROFILE_DIR}}` 在启动前替换进 `command`/`args`。消息只通过 stdin 传递，不放入 argv。
...
- **范围。** `env_allowlist` 仅治理 profile `env` 块内的 `${VAR}` 展开。它不影响 infra 集（见下）、`{{SESSION_ID}}` / `{{PROFILE_DIR}}` 占位符，或完全不含 `${VAR}` 的 `env` 值。
...
结果 argv（`[command, *args]`）传给平台的 `execve`（或等价物），**不调用 shell**。这让 `command` 能携带含空格的路径。
...
5. 以与 spawn 路径相同的语义应用 `timeout_secs` / `kill_grace_secs` / `streaming` / `permission`。
...
线上的 `session_id` **MUST** 是非空 JSON 字符串，且不含路径分隔符（`/`、`\`）和控制字符（含 NUL 与换行）。参考 SDK runner 在线上分类阶段就执行此约束：不合规的 `session_id` 会被静默丢弃并记 stderr 警告（已捕获的 id（若有）保持不变）。约束之所以在 runner 层而非仅在存储层落地，原因在于 SDK 历史助手把会话持久化为 `<id>.jsonl` 文件——含 `/` 的 `session_id` 会静默创建子目录而非平铺文件。冒号、空格、连字符等大多数可打印字符均被接受（例如 `"org:proj:thread-42"` 合法；`"org/proj/thread"` 不合法）。
...
**SDK 约定。** 在参考 SDK（`create_profile`/`createProfile`）中，`send_error()` / `sendError()` 是**非终止性**的：发出 `{"type":"error"}` 事件后，handler **MAY** 继续执行并返回正文。`error` 事件与后续 `result` 事件都会出现在 stdout；bridge 会丢弃该 `result`（见上方规则），因此用户只看到错误。如需真正致命的错误路径，handler 应抛出 / throw `ProtocolError` / `protocolError()`，它会以非零退出码终止进程。
...
其他非零码视为通用错误。进程非零退出且未发 `error` 事件时，bridge **SHOULD** 向用户发送一条通用错误消息。
...
以下行为是 bridge 侧的部署决策。它们**不是**参考 SDK runner 会读取的 profile YAML 字段；调用方通过 `RunResult.error` / `RunResult.exitCode` 和 `onStderr` 拿到原始数据后自行决定如何呈现。

- **向用户报告错误**（SHOULD）：当 turn 失败（非零退出或 `error` 事件）时，bridge **SHOULD** 向用户转发错误消息，而非保持沉默。
- **stderr 可见性**（默认隐藏）：stderr 作为内部调试日志捕获。开发/调试场景下 bridge **MAY** 展示给用户；生产部署 **SHOULD** 默认隐藏。
